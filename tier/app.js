// Per-page config must be set by the page's own app.js before this file loads.
const _cfg = window.TIER_PAGE_CONFIG || {};
const COLLECTION_ID = _cfg.collectionId || "";
const COVER_CACHE_KEY = _cfg.coverCacheKey || "tier_cover_cache_v1";
...
function renderList() {
  ...
  if (!filtered.length) {
    ...
    if (state.query) {
      ...
      setStatus(`No ${ITEM_LABEL_LC} found.`);
    }
    return;
  }
