...
const ANIME_COLLECTION_ID = "6a02d598001305384d8b";
const PAGE_SIZE = 100;
const THEME_KEY = "darkMode";
const COVER_CACHE_KEY = "series_cover_cache_v1";
const ANIME_WORKER_BASE = "https://anime.kampfflugzeuge.workers.dev";
const JIKAN_PROXY_BASE = `${ANIME_WORKER_BASE}/jikan`;
const APPWRITE_PROXY_URL = `${ANIME_WORKER_BASE}/appwrite/serien`;
...
function renderCardCover(record) {
  ...
  const safeTitle = escapeHtml(record.title || "Series");
...
async function enrichVisibleCovers(records, jobId) {
  ...
  for (let i = 0; i < pending.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (record) => {
      ...
      if (slot) {
        slot.innerHTML = `<img class="card-cover" src="${escapeHtml(cover)}" alt="${escapeHtml(record.title || "Series")} cover" loading="lazy" referrerpolicy="no-referrer" />`;
      }
    }));
...
async function fetchAnimeRankingDirect() {
  ...
  while (true) {
    ...
    if (safetyCounter > 1000) {
      throw new Error("Pagination safety limit reached while reading series.");
    }
  }
...
function renderList() {
  ...
  if (!filtered.length) {
    renderEmpty("No series found for the current search.");
    if (state.query) {
      setStatus(`Showing 0 of ${state.records.length} series.`);
    } else {
  ...
  if (unranked.length > 0) {
    ...
    html += '<div class="unranked-items unranked-empty"><p class="unranked-empty-text">No unranked series.</p></div>';
  }
  html += '</div>';
  ...
  if (state.query) {
    setStatus(`Showing ${filtered.length} of ${state.records.length} series.`);
  } else {
    setStatus(`${state.records.length} series loaded.`);
  }
...
async function deleteRecord(id) {
  ...
  try {
    ...
    renderList();
    setStatus("Series entry deleted.");
  } catch (error) {
    const msg = error?.message || "";
    const isPermissionError = msg.includes("Missing") && msg.includes("permission");
...
async function loadAnimeList() {
  ...
  try {
    ...
    if (result.invalid.length > 0) {
      console.warn("Skipped invalid series documents:", result.invalid);
    }
  } catch (error) {
    state.records = [];
    renderEmpty(error?.message || "Unable to load series list.");
    setStatus("Failed to load series list.");
  } finally {
    if (elements.refresh) {
...
function openEditor(record) {
  ...
  if (elements.editTitleText) {
    elements.editTitleText.textContent = state.activeEditId ? "Edit Series" : "Add Series";
  }
...
async function saveEditor(event) {
  ...
  const successText = editId ? "Series entry updated." : "Series entry created.";
  ...
  try {
    ...
    const msg = error?.message || "";
    const isPermissionError = msg.includes("Missing") && msg.includes("permission");
    setEditError(isPermissionError ? "You don't have permission to add series entries." : (msg || "Could not save series entry."));
    setEditLoading(false);
  }
}
...
function handleListClick(event) {
  ...
  if (quickEditButton) {
    ...
    if (!record) { setStatus("Could not find that series entry. Refresh and try again."); return; }
...
function performExport() {
  ...
  a.href = url;
  a.download = `series-ranking.${extension}`;
  a.click();
